...
export async function getNetworkAddress(networks) {
  const _chainId = chainId ? BigNumber.from(chainId).toNumber() : chainId;

  if (!networks[_chainId] || !networks[_chainId].address) return null;
  return networks[_chainId].address;
}
...
function handleChainChanged(_chainId) {
  ...
  chainId = _chainId;
  console.log(
    "Ethereum chain has changed. We will reload the page as recommended."
  );
